...
fetch_manifest() {
    ...
    if curl -sf --max-time 15 -o "${MANIFEST_FILE}" "${GITHUB_RAW}/manifest.txt" \
        ...
        cat > "${MANIFEST_FILE}" <<'MANIFEST_FALLBACK'
...
bin scripts/wg-set-dns
bin scripts/wg-set-ddns
bin scripts/wg-set-update-policy
bin scripts/wg-update
bin scripts/wg-check-update
bin scripts/wg-reset
web dashboard/bootstrap.php
...
setup_backup() {
    ...
    chmod 700 "${WGM_DIR}/backups"

    # Deploy the canonical (hardened) backup script from the repo. wg-update keeps
    # it current. Fall back to a stub if GitHub is unreachable, so the cron logs an
    # honest error instead of silently doing nothing.
    local btmp
    btmp="$(mktemp)"
    if curl -sf --max-time 30 -o "${btmp}" "${GITHUB_RAW}/scripts/backup.sh" \
        && [[ -s "${btmp}" ]] && head -1 "${btmp}" | grep -q '^#!'; then
        mv "${btmp}" "${WGM_BACKUP_SCRIPT}"
        print_success "Backup script installed."
    else
        rm -f "${btmp}"
        cat > "${WGM_BACKUP_SCRIPT}" <<'STUB'
#!/usr/bin/env bash
echo "$(date) [ERROR] backup.sh was not downloaded during install; run 'sudo wg-update'." \
    >> /var/log/wireguard-manager/backup.log
exit 1
STUB
        print_warn "backup.sh not downloaded — run 'wg-update' to fetch it."
    fi
    chmod 700 "${WGM_BACKUP_SCRIPT}"

    # Schedule daily backup at 2:00 AM (run directly; matches the sudoers rule)
    echo "0 2 * * * root ${WGM_BACKUP_SCRIPT}" > /etc/cron.d/wireguard-backup
...
setup_dashboard() {
    ...
    # Write a private-by-default vhost inline (HTTP:80, LAN + VPN ranges only). This
    # is the safe fallback that always works. If the user opted into public access,
    # wg-dashboard-access is invoked further below to switch on HTTPS — that script
    # is the single authority for vhost/HTTPS config (see end of this function).
...
SUDOERS
    ...
    # Dashboard audit log — the web user must be able to append to it, so it is
    # group-owned by www-data (install.log stays root-only).
    touch "${WGM_LOG_DIR}/dashboard.log"
    chown root:www-data "${WGM_LOG_DIR}/dashboard.log"
    chmod 664 "${WGM_LOG_DIR}/dashboard.log"

    # If public access was chosen, switch on HTTPS now (wg-dashboard-access builds
    # the self-signed cert + SSL vhost + 80→443 redirect). Falls back to the private
    # HTTP vhost written above if the helper is unavailable.
    if [[ "${DASHBOARD_PUBLIC:-false}" == true ]]; then
        if "${BIN_DIR}/wg-dashboard-access" public >/dev/null 2>&1; then
            print_success "Public HTTPS enabled (self-signed certificate)."
        else
            print_warn "Could not enable public HTTPS automatically — dashboard left private (LAN + VPN). Switch it from the dashboard later."
        fi
    fi
...
print_completion() {
    ...
    if [[ "${INSTALL_BACKUP}" == true ]]; then
        ...
        echo -e "  ${CYAN}Manual   :${RESET} sudo ${WGM_BACKUP_SCRIPT}"
    fi
